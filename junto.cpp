...
public:
    // Define a quantidade de bolas padrão do computador (15)
    computador()
    {
        qBolasc = 15;
    }
    int obtemNumBolasC() { return qBolasc; }
    void defineNumBolasC(int d) { qBolasc = d; }
};
...
public:
    ...
    int obtemNumBolasJ() { return qBolasj; } // Obtem número de bolas que o jogador tem
    void defineNumBolasJ(int d) { qBolasj = d; }
...
public:
    ...
    int obtemCenter() { return center; }
    void defineCenter(int c) { center = c; }
    string str()
    {
        stringstream ss;
        ss << endl
...
void execute::comeca() {
    ...
    while (1) {
        ...
        //apos cada rodada, o tabuleiro é imprimido
        cout << tab.str() << endl;
        this_thread::sleep_for(chrono::seconds(2));
    }
}
